...
// On Railway, /tmp is writable (repo directory may be read-only)
...
// ===== PAGE GUARDS (optional, but recommended) =====
// If you already have your own landing/login pages, keep these routes.
...
// Telemetry from ESP32 (optional for now; good later)
app.post("/api/v1/stations/:id/telemetry", (req, res) => {
  ...
  if (typeof p.lat === "number") s.lat = p.lat;
  if (typeof p.lng === "number") s.lng = p.lng;
  ...
  // enforce policy
  if ((s.fillPct || 0) >= 95) s.locked = true;
...
app.post("/api/rewards/scan", requireResident, (req, res) => {
  ...
  // Anti-abuse: 1 scan per station per hour
...
// ===== INVESTOR DEMO: OVERFLOW SIMULATION ENGINE =====
// This makes the map feel alive (fills up, alerts, locks, heatmap changes)

let simulationOn = false;

// Optional: “wave center” for drama (your pilot center)
const WAVE_CENTER = { lat: 26.3188612, lng: 50.2277122 };

function distMetersApprox(a, b) {
  // fast approx distance (good enough for demo)
  const dLat = (b.lat - a.lat);
  const dLng = (b.lng - a.lng);
  return Math.sqrt(dLat * dLat + dLng * dLng);
}

function simulateOverflowTick() {
  ...
  stations.forEach((s, i) => {
    ...
    // wave: stations closer to center fill faster
    const d = distMetersApprox({ lat: Number(s.lat), lng: Number(s.lng) }, WAVE_CENTER);
    const centerBoost = Math.max(0, 1.6 - (d * 650)); // scale for your lat/lng degrees

    const wave = 1.6 + Math.sin(now / 4500 + i) * 1.1; // oscillation
    const random = Math.random() * 1.2;

    let inc = wave + random + centerBoost;

    // locked stations stay locked until emptied (realistic)
    if (s.locked) inc = 0.2;

    s.fillPct = Math.min(100, s.fillPct + inc);

    // rules
    if (s.fillPct >= 80) s.odorAlert = true;
    if (s.fillPct >= 95) s.locked = true;
...
// 4 seconds feels “live” in demos
setInterval(simulateOverflowTick, 4000);

app.post("/api/simulation/start", requireAuthority, (req, res) => {
  simulationOn = true;
  res.json({ ok: true });
});

app.post("/api/simulation/stop", requireAuthority, (req, res) => {
  simulationOn = false;
  res.json({ ok: true });
});

app.post("/api/simulation/reset", requireAuthority, (req, res) => {
  const db = readDB();
  (db.stations || []).forEach((s, i) => {
    // reset to low baseline with slight variation
    s.fillPct = 8 + (i % 7) * 2;
    s.odorAlert = false;
    s.locked = false;
    s.lastSeen = new Date().toISOString();
